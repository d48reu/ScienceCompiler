...
OUTDIR = ROOT / 'hackathon' / 'demo_video'
...
def draw_terminal_slide(spec: dict, idx: int) -> Path:
    ...
    if idx == 1:
        ...
        for x in range(0, W, 42):
            alpha = 20 if x % 84 == 0 else 10
            d.line((x, 0, x, H), fill=(30, 220, 200, alpha))
        for y in range(0, H, 42):
            alpha = 16 if y % 84 == 0 else 8
            d.line((0, y, W, y), fill=(30, 220, 200, alpha))

        d.rounded_rectangle((58, 70, W - 58, H - 70), radius=28, fill=(14, 18, 26), outline=(70, 88, 110), width=2)
        d.rounded_rectangle((88, 112, 500, 168), radius=16, fill=(26, 34, 46))
        d.text((110, 126), 'NOUS RESEARCH HACKATHON', font=SMALL_FONT, fill=ACCENT)

        d.text((92, 228), spec['title'], font=ImageFont.truetype(FONT_PATH, 68), fill=TEXT)
        d.text((95, 312), spec['subtitle'], font=ImageFont.truetype(FONT_PATH, 28), fill=ACCENT2)

        d.rounded_rectangle((90, 390, W - 90, 570), radius=20, fill=(20, 26, 36), outline=BORDER, width=2)
        y = 420
        for line, color in spec['body']:
            d.text((120, y), line, font=BODY_FONT, fill=color)
            y += 36

        right_x0, right_y0 = 830, 180
        right_x1, right_y1 = 1160, 560
        d.rounded_rectangle((right_x0, right_y0, right_x1, right_y1), radius=18, fill=(18, 24, 34), outline=(94, 234, 212), width=2)
        d.text((860, 215), '> pipeline', font=SUB_FONT, fill=ACCENT)
        d.text((860, 260), 'papers', font=BODY_FONT, fill=TEXT)
        d.text((860, 295), '  -> claims', font=BODY_FONT, fill=TEXT)
        d.text((860, 330), '  -> contradictions', font=BODY_FONT, fill=TEXT)
        d.text((860, 365), '  -> weighted evidence', font=BODY_FONT, fill=TEXT)
        d.text((860, 400), '  -> experiments', font=BODY_FONT, fill=TEXT)
        d.text((860, 460), 'domain 1:', font=SMALL_FONT, fill=MUTED)
        d.text((860, 486), 'butyrate / neuroinflammation', font=SMALL_FONT, fill=TEXT)
        d.text((860, 520), 'domain 2:', font=SMALL_FONT, fill=MUTED)
        d.text((860, 546), 'senolytics / aging', font=SMALL_FONT, fill=TEXT)

        d.text((96, H - 112), spec['footer'], font=SMALL_FONT, fill=MUTED)
    else:
        margin = 48
    ...
    img.save(path)
    if idx == 1:
        img.save(ROOT / 'hackathon' / 'science_compiler_thumbnail.png')
    return path
...
def main() -> None:
    ...
    out_video = ROOT / 'hackathon' / 'science_compiler_nous_demo.mp4'
